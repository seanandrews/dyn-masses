...
#
df = np.load('fake_data/data_uvfits/V892Tau.freq_conversions.npz')
...
FOV, dist, Npix, Tbmax, r0 = 6.0, 130., 256, 700., 10.
...
def lnprob(theta):
    ...
    ptheta[0] = lnpNU(theta[0], 54.5, 2., 0., 90.)
    ptheta[1] = lnpNU(theta[1], 52.1, 2., 0., 360.)
    ptheta[2] = lnpU(theta[2], 0., 10.)
    ptheta[3] = lnpNU(theta[3], 230., 30., r0, 0.5*(dist * FOV))
    ptheta[4] = lnpNU(theta[4], 1.0, 0.2, 0.1, 10.)
    ptheta[5] = lnpNU(theta[5], 1.0, 0.2, 0., 1.5)
    ptheta[6] = lnpU(theta[6], 5., Tbmax)
    ptheta[7] = lnpU(theta[7], 0., 2.)
    ptheta[8] = lnpNU(theta[8], 20., 2., 5., 50.)
    ptheta[9] = lnpNU(theta[9], 8.2, 0.1, 7.5, 9.0)
    ptheta[10] = lnpNU(theta[10], -0.13, 0.02, -0.25, 0.0)
    ptheta[11] = lnpNU(theta[11],  0.15, 0.02, 0.0, 0.25)
